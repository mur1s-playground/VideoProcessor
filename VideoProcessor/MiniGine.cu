        ...
        for (int e = 0; e < entities_count; e++) {
            ...
            if (entity_id < UINT_MAX) {
                ...
                if (offset_to_model_base_x >= entities[entity_id].crop_x[0] && offset_to_model_base_x < entities[entity_id].crop_x[1] &&
                                ...
                                unsigned char* p_model = (unsigned char*)&device_data_assets[model_positions_in_bf[((int)(entities[entity_id].orientation / 10 / (36 / m->model_rotations)) % m->model_rotations) * m->model_animation_ticks + animation_tick]];
                                
                                for (int s_y = 0; s_y < 1; s_y++) {
                                    for (int s_x = 0; s_x < 1; s_x++) {
                                        if (offset_to_model_base_x + s_x >= entities[entity_id].crop_x[0] && offset_to_model_base_x + s_x < entities[entity_id].crop_x[1] &&
                                            ...
                                            if (interpixel_alpha > 0) {
                                                if ((entities[entity_id].position[1] + (m->model_dimensions[1] * entities[entity_id].scale) > player_y_max && entities[entity_id].model_z == player_z_max) || entities[entity_id].model_z > player_z_max) {
                                                    player_z_max = entities[entity_id].model_z;
                                                    player_y_max = entities[entity_id].position[1] + (m->model_dimensions[1] * entities[entity_id].scale);
...
void mini_gine_draw_entities_kernel_launch(const unsigned int* device_data_assets, const unsigned int models_position,
    ...
    mini_gine_draw_entities_kernel << <blocksPerGrid, threadsPerBlock, 0, cuda_streams[1] >> > (device_data_assets, models_position, device_data_rw, entities_position, entities_count,
